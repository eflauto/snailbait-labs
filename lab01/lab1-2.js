...
// Draw runner in different positions (plural). We will draw the runner at the center of each platform. - EF
function drawRunner() {
    let runnerLeftPos;
    let runnerTopPos;

    for (let index = 0; index < platformData.length; index++) {
        runnerLeftPos =  platformData[index].left + (platformData[index].width / 2) - (runnerImage.width / 2);
        runnerTopPos = calculatePlatformTop(platformData[index].track) - runnerImage.height;

        context.drawImage(runnerImage, runnerLeftPos, runnerTopPos);
    }
}
